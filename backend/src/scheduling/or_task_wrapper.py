...
class ORTaskWrapper:
    ...
    def __init__(self, task_obj, model, period_start_dt, period_end_dt_minutes):
        ...
        # Store priority for penalty calculation
        self.priority = task_obj.priority

        # This boolean is TRUE if the task is performed/scheduled.
        self.is_performed_bool = model.NewBoolVar(f"is_performed_{self.id}")

        # The interval variable is now optional. It only exists if is_performed_bool is true.
        self.interval_var = model.NewOptionalIntervalVar(
            self.start_var,
            self.duration_min,
            self.end_var,
            self.is_performed_bool,  # Link interval to the boolean variable
            f"interval_{self.id}",
        )
