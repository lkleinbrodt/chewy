...
def cleanup_past_recurring_tasks():
    """Remove any recurring tasks that are in the past."""
    today = datetime.utcnow().date()
    past_tasks = Task.query.filter(
        Task.recurring_event_id.isnot(None), Task.instance_date < today
    ).all()
    ...
    if past_tasks:
        db.session.commit()
        print(f"Cleaned up {len(past_tasks)} past recurring tasks")
...
def reset_recurring_events(start_date, end_date):
    ...
    # Clean up any existing past recurring tasks first
    cleanup_past_recurring_tasks()
