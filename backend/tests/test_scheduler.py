...
class TestSchedulerBasics:
    def test_empty_schedule(self, app, test_db, date_range):
    ...
    def test_rounding_all_tasks(self, app, test_db, date_range, create_task_factory):
        """Test that all tasks are rounded to the nearest 5 minutes"""
        start_date, end_date = date_range()
        with app.app_context():
            # Create multiple tasks with durations that would result in non-5-minute-aligned times
            tasks = []
            for i in range(3):
                tasks.append(
                    create_task_factory(
                        content=f"Test Rounding Task {i}",
                        duration=47 + i,  # 47, 48, 49 minutes
            ...
            # Check that all tasks have rounded times
            for task in scheduled_tasks:
                ...
                ), f"Start time {start_time} not rounded to 5 minutes"
                assert (
                    end_time.minute % 5 == 0
                ), f"End time {end_time} not rounded to 5 minutes"

                # The duration should still be approximately correct (within 5 minutes)
                original_task = next(t for t in tasks if t.id == task["task_id"])
                actual_duration = (end_time - start_time).total_seconds() / 60
                assert (
                    abs(actual_duration - original_task.duration) <= 5
                ), f"Duration {actual_duration} too far from expected {original_task.duration} minutes"

    def test_basic_task_scheduling(self, app, test_db, date_range, create_task_factory):
    ...
    def test_recurring_task(self, app, test_db, date_range):
        ...
        with app.app_context():
            ...
            recurring_task = RecurringEvent(
                ...
                time_window_end=create_time(13, 0),
            )
            test_db.session.add(recurring_task)
            test_db.session.commit()

            # add some calendar events and some other random tasks
            for i in range(4):
                event = CalendarEvent(
                    subject=f"Event {i}",
                    start=datetime.combine(start_date.date(), create_time(10, 0)),
            ...
            # and that the start time is within the time window
            scheduled_recurring_tasks = []
            for task in scheduled_tasks:
                for rt in recurring_tasks:
                    if task["task_id"] == rt.id:
                        assert task["start"].hour >= rt.time_window_start.hour
                        assert task["start"].hour <= rt.time_window_end.hour
                        print(
                            f"Scheduled recurring task {task['task_id']} at {task['start']}"
                        )
...
class TestTimeConstraints:
    ...
    def test_task_specific_time_window(
        self, app, test_db, date_range, create_task_factory
    ):
...
class TestRecurringEvents:
    def test_recurring_daily_task(
        self, app, test_db, date_range, create_recurring_event_factory
    ):
            ...
            for task in scheduled_tasks:
                for rt in recurring_tasks:
                    if task["task_id"] == rt.id:
                        ...
                        days.append(task["start"].date())
                        break
            print(days)

            assert scheduled_recurring_count == len(recurring_tasks)
            assert len(days) == len(set(days))
            assert len(days) == len(recurring_tasks)

    def test_recurring_task_without_time_window(
        self, app, test_db, date_range, create_recurring_event_factory
    ):
        """Test that recurring tasks without time windows are scheduled on their instance_date"""
        start_date, end_date = date_range(days=7)  # Ensure we cover a full week

        with app.app_context():
            # Create a recurring task with no time window
            recurring_event = create_recurring_event_factory(
                content="Recurring Task Without Time Window",
                duration=45,  # 45 minutes
                recurrence=[0, 2, 4],  # Monday, Wednesday, Friday
                # Explicitly not setting time_window_start and time_window_end
            )
            ...
            # Check if scheduling was feasible
            assert status_message == "Feasible"
            assert scheduled_tasks is not None

            # Verify all recurring tasks were scheduled on their instance_date
            for task in scheduled_tasks:
                for rt in recurring_tasks:
                    if task["task_id"] == rt.id:
                        # The scheduled task start date should match the instance_date
                        assert task["start"].date() == rt.instance_date
                        # Verify the weekday is either Monday (0), Wednesday (2), or Friday (4)
                        assert task["start"].date().weekday() in [0, 2, 4]
                        break

            # Ensure all recurring tasks were scheduled
            scheduled_recurring_ids = [
                task["task_id"]
                for task in scheduled_tasks
                if any(task["task_id"] == rt.id for rt in recurring_tasks)
            ]
            assert len(scheduled_recurring_ids) == len(recurring_tasks)
...
class TestSchedulerIntegration:
    def test_complex_schedule_with_all_constraints(
        ...
        start_date, end_date = date_range(days=7)  # Full week
...
class TestSchedulerObjectives:
    """Test suite for scheduler objectives and reward functions."""

    def test_free_morning_objective(
        self, app, test_db, date_range, create_task_factory
    ):
        """Test that FreeMorningReward objective successfully delays task start times."""
        start_date, end_date = date_range(days=1)  # Just test one day for simplicity

        with app.app_context():
            # Create several tasks that could be scheduled any time during the day
            tasks = []
            for i in range(3):
                tasks.append(
                    create_task_factory(
            ...
            # First, generate schedule without the objective
            objectives_disabled = (
                SchedulerObjectives.default()
            )  # All objectives disabled by default
            result_without_obj = generate_schedule(
                start_date, end_date, objectives=objectives_disabled
            )
            scheduled_tasks_without_obj = result_without_obj["scheduled_tasks"]
            status_message = result_without_obj["status_message"]
            assert status_message == "Feasible"

            # Then, generate schedule with FreeMorningReward objective enabled
            objectives_enabled = SchedulerObjectives.default()
            objectives_enabled.enable_objective("FREE_MORNING", weight=1)
            result_with_obj = generate_schedule(
                start_date, end_date, objectives=objectives_enabled
            )
            scheduled_tasks_with_obj = result_with_obj["scheduled_tasks"]
            status_message = result_with_obj["status_message"]
            assert status_message == "Feasible"

            # Find earliest task start time in each schedule
            earliest_start_without_obj = min(
                task["start"].hour for task in scheduled_tasks_without_obj
            )
            earliest_start_with_obj = min(
                task["start"].hour for task in scheduled_tasks_with_obj
            )

            # The schedule with FREE_MORNING objective should start tasks later
            assert earliest_start_with_obj > earliest_start_without_obj, (
                f"Expected tasks to start later with FREE_MORNING objective, but got "
                f"earliest start time {earliest_start_with_obj}:00 with objective vs "
                f"{earliest_start_without_obj}:00 without objective"
            )

    def test_even_workload_objective(
        self, app, test_db, date_range, create_task_factory
    ):
        """Test that EvenWorkloadReward objective successfully distributes tasks across days."""
        # Use a 3-day period for this test
        start_date, end_date = date_range(days=3)

        with app.app_context():
            # Create several tasks with different durations
            tasks = []
            for i in range(6):
                # Make a mix of short and long tasks
                duration = 60 if i % 2 == 0 else 120  # 1 hour or 2 hours
                tasks.append(
                    create_task_factory(
                        content=f"Task {i}",
                        duration=duration,
                        due_by=valid_due_date(start_date, days=3),
                    )
                )

            # First, generate schedule without the objective
            objectives_disabled = SchedulerObjectives.default()
            result_without_obj = generate_schedule(
                start_date, end_date, objectives=objectives_disabled
            )
            scheduled_tasks_without_obj = result_without_obj["scheduled_tasks"]
            ...
            # Then, generate schedule with EvenWorkloadReward objective enabled
            objectives_enabled = SchedulerObjectives.default()
            objectives_enabled.enable_objective(
                "EVEN_WORKLOAD", weight=10
            )  # Higher weight to prioritize
            result_with_obj = generate_schedule(
                start_date, end_date, objectives=objectives_enabled
            )
            scheduled_tasks_with_obj = result_with_obj["scheduled_tasks"]
            ...
            # Calculate work hours per day for both schedules
            def calculate_work_hours_per_day(schedule):
                work_by_day = {}
                for task in schedule:
                    task_start = task["start"]
                    task_end = task["end"]
                    task_date = task_start.date()

                    # Handle tasks spanning multiple days
                    if task_start.date() != task_end.date():
                        # For simplicity, just count the task on its start date
                        # In a real scenario, we'd need to apportion the time across days
                        pass

                    if task_date not in work_by_day:
                        work_by_day[task_date] = 0

                    # Add the task's duration in hours
                    task_duration = (task_end - task_start).total_seconds() / 3600
                    work_by_day[task_date] += task_duration

                return work_by_day

            work_hours_without_obj = calculate_work_hours_per_day(
                scheduled_tasks_without_obj
            )
            work_hours_with_obj = calculate_work_hours_per_day(scheduled_tasks_with_obj)

            # Calculate standard deviation for both work distributions
            def calculate_std_dev(work_hours):
                if not work_hours:
                    return 0

                values = list(work_hours.values())
                mean = sum(values) / len(values)
                variance = sum((x - mean) ** 2 for x in values) / len(values)
                return variance**0.5

            std_dev_without_obj = calculate_std_dev(work_hours_without_obj)
            std_dev_with_obj = calculate_std_dev(work_hours_with_obj)

            # The schedule with EVEN_WORKLOAD objective should have a lower standard deviation
            # (more evenly distributed work)
            assert std_dev_with_obj <= std_dev_without_obj, (
                f"Expected work to be more evenly distributed with EVEN_WORKLOAD objective, "
                f"but got std dev {std_dev_with_obj} with objective vs "
                f"{std_dev_without_obj} without objective"
            )

            # Log the work distribution for debugging


class TestResilientScheduler:
    def test_drop_task_due_in_past(self, app, test_db, date_range, create_task_factory):
        """Test that a task with impossible constraints is immediately dropped."""
        start_date, end_date = date_range()
        with app.app_context():
            # Create a task that is impossible to schedule due to time window constraints
            impossible_task = create_task_factory(
                content="Impossible Task",
                duration=120,  # 2 hours
                due_by=end_date,  # Due within the scheduling period
                time_window_start=create_time(14, 0),  # 2 PM
                time_window_end=create_time(15, 0),  # 3 PM - only 1 hour window
            )
            db.session.commit()
            ...
            print(f"Task due_by: {impossible_task.due_by}")
            print(f"Task duration: {impossible_task.duration}")
            print(
                f"Task time window: {impossible_task.time_window_start} - {impossible_task.time_window_end}"
            )
            print(f"Scheduled tasks: {len(result['scheduled_tasks'])}")
            print(f"Unscheduled tasks: {len(result['unscheduled_tasks'])}")
            if result["unscheduled_tasks"]:
                print(
                    f"First unscheduled task reason: {result['unscheduled_tasks'][0]['reason']}"
                )
            ...
            assert failure_info["task"].id == impossible_task.id
            assert "longer than its time window" in failure_info["reason"].lower()

            # Verify DB status
            db_task = db.session.get(Task, impossible_task.id)
            assert db_task.status == "unschedulable"

    def test_partial_schedule_with_resource_conflict(
        self,
        app,
        test_db,
        date_range,
        ...
        with app.app_context():
            ...
            create_calendar_event_factory(
                ...
                start=datetime.combine(
                    start_date.date(), create_time(work_start_hour + 1, 0)
                ),
                end=datetime.combine(
                    start_date.date(), create_time(app.config["WORK_END_HOUR"], 0)
                ),
            )
    ...
    def test_cascading_failure_from_dependency(
        self,
        app,
        test_db,
        date_range,
        ...
        with app.app_context():
            # Task A is impossible to schedule (its time window is fully blocked on both days)
            # Block the first day
            blocked_time_day1 = datetime.combine(start_date.date(), create_time(14, 0))
            create_calendar_event_factory(
                subject="Blocker Day 1",
                start=blocked_time_day1,
                end=blocked_time_day1 + timedelta(hours=2),
            )

            # Block the second day
            blocked_time_day2 = datetime.combine(
                (start_date + timedelta(days=1)).date(), create_time(14, 0)
            )
            create_calendar_event_factory(
                subject="Blocker Day 2",
                start=blocked_time_day2,
                end=blocked_time_day2 + timedelta(hours=2),
            )
            ...
            print(
                f"Blocked time: {blocked_time_day1} to {blocked_time_day1 + timedelta(hours=2)}"
            )
            print(
                f"Task A time window: {task_a.time_window_start} - {task_a.time_window_end}"
            )
            print(f"Task A duration: {task_a.duration}")
            print(f"Scheduled tasks: {len(result['scheduled_tasks'])}")
            print(f"Unscheduled tasks: {len(result['unscheduled_tasks'])}")
            if result["scheduled_tasks"]:
                for task in result["scheduled_tasks"]:
                    print(
                        f"Scheduled task {task['task_id']}: {task['start']} - {task['end']}"
                    )
            if result["unscheduled_tasks"]:
                for task in result["unscheduled_tasks"]:
                    print(f"Unscheduled task {task['task'].id}: {task['reason']}")
