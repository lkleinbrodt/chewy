...
def sync_calendar():
    ...
    # Get state of non-Chewy-managed events before sync
    events_before_query = CalendarEvent.query.filter(
        CalendarEvent.is_chewy_managed == False
    )
    # Using a set of tuples for comparison. Ensure datetimes are consistently represented.
    events_before_state = {
        (event.id, event.subject, event.start.isoformat(), event.end.isoformat())
        for event in events_before_query.all()
    }
    ...
    # NEW: Counter for Chewy-managed events skipped during import
    chewy_events_skipped = 0
        ...
        try:
            with open(file_path, "r") as f:
                ...
                for event_data in events_data:
                    ...
                    # NEW: Skip any event that is already a Chewy Scheduled event
                    categories = event_data.get("categories", [])
                    if any("chewy scheduled" in cat.lower() for cat in categories):
                        chewy_events_skipped += 1
                    ...
                    # This check is now redundant because of the skip logic above, but it's harmless
                    is_chewy_managed = any("chewy" in cat.lower() for cat in categories)

                    # Check if event already exists
                    event = db.session.get(CalendarEvent, event_data["id"])
    ...
    # Get state of non-Chewy-managed events after sync
    events_after_query = CalendarEvent.query.filter(
        CalendarEvent.is_chewy_managed == False
    )
    events_after_state = {
        (event.id, event.subject, event.start.isoformat(), event.end.isoformat())
        for event in events_after_query.all()
    }

    # Check if any non-Chewy events were changed
    changes_detected = events_before_state != events_after_state

    # If events were deleted, that's also a change
    if current_event_ids - synced_event_ids:
        deleted_non_chewy_events = CalendarEvent.query.filter(
            CalendarEvent.id.in_(current_event_ids - synced_event_ids),
            CalendarEvent.is_chewy_managed == False,
        ).count()
        if deleted_non_chewy_events > 0:
    ...
    return jsonify(
        {
            ...
            "chewy_events_skipped": chewy_events_skipped,  # NEW
            "changes_detected": changes_detected,  # New field
        }
    )
