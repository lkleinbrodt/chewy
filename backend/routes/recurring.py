...
from backend.models.task import RecurringEvent, Task
from backend.src.utils import parse_iso_datetime

logger = create_logger(__name__, level="INFO")

recurring_bp = Blueprint("recurring", __name__, url_prefix="/recurring-events")
...
def extract_time_window(event_data):
...
def create_recurring_event():
    ...
    time_window_start, time_window_end = extract_time_window(event_data)
...
def update_recurring_event(recurring_event_id):
    ...
    time_window_start, time_window_end = extract_time_window(data)
    # allow it to be None
    event.time_window_start = time_window_start
    event.time_window_end = time_window_end
