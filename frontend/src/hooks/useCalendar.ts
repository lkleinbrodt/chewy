...
export function useCalendar(initialDate = new Date()) {
  // If today is a weekend, use next Monday as the initial date
  const initialCalendarStartDate = useMemo(() => {
    return isWeekend(initialDate) ? nextMonday(initialDate) : initialDate;
  }, [initialDate]);

  const [calendarStartDate, setCalendarStartDate] = useState<Date>(
    initialCalendarStartDate
  );
  ...
  const { startDate, endDate } = useMemo(() => {
    ...
    const start = startOfWeek(calendarStartDate, { weekStartsOn: startDay });
  ...
  const nextWeek = () => {
    setCalendarStartDate((prev) => addWeeks(prev, 1));
  };

  // Navigate to previous week
  const prevWeek = () => {
    setCalendarStartDate((prev) => subWeeks(prev, 1));
  };
  ...
  const syncCalendar = async () => {
    try {
      ...
      setError(null);
      return { success: true, changes_detected: result.changes_detected };
    } catch (err) {
